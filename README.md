...
- **One accent color** drives the look — set `--accent` / `--accent2` in `:root` and reuse it. Pick a distinct color per plan (see the palette note below).
...
const PLANS = [
  {
    ...
    blurb:  "A cheap, sturdy, splinter-free outdoor mud kitchen…",  // 1–2 sentences
